...
uploadBuildRun(){
	...
	then
		...
		echo "using rsync..."
		rsync -ac --out-format="   %n" --no-t --delete-after --exclude=$BBB_PROJECT_NAME --exclude=startup --exclude=sourcetrail --exclude=build $HOST_SOURCE_PATH"/" "$BBB_NETWORK_TARGET_FOLDER/" | tee | checkIfOnlyHeaders #trailing slashes used here make sure rsync does not create another folder inside the target folder
	fi
